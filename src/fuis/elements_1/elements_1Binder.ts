/** This is an automatically generated class by FairyGUI. Please do not modify it. **/

module fuis.elements_1 {

	export class elements_1Binder{
		public static bindAll():void {
			fairygui.UIObjectFactory.setPackageItemExtension(UI_InvincibleEffect.URL, UI_InvincibleEffect);
			fairygui.UIObjectFactory.setPackageItemExtension(UI_TankAvatar.URL, UI_TankAvatar);
			fairygui.UIObjectFactory.setPackageItemExtension(UI_Tank.URL, UI_Tank);
			fairygui.UIObjectFactory.setPackageItemExtension(UI_MapCell.URL, UI_MapCell);
			fairygui.UIObjectFactory.setPackageItemExtension(UI_Bullet.URL, UI_Bullet);
		}
	}
}